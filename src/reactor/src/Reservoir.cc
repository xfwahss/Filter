...
double Reservoir::predict_volumn(const double &seconds, const double &flow_in,
                                 const double &flow_out) {
    double volumn = this->volumn + (flow_in - flow_out) * seconds * 1e-8;
    return volumn;
}

double Reservoir::predict_organic(const double &seconds, const double &load_in,
                                  const double &load_out, const double &ro,
                                  const double &next_volumn) {
    double c_v = status.c_no * volumn + (load_in - load_out) * seconds * 1e-8 -
                 ro * volumn * seconds;
    double co = c_v / next_volumn;
    return co;
}

double Reservoir::predict_ammonia(const double &seconds, const double &load_in,
                                  const double &load_out, const double &ro,
                                  const double &ra, const double &next_volumn) {
    double c_v = status.c_na * volumn + (load_in - load_out) * seconds * 1e-8 -
                 ra * volumn * seconds + ro * volumn * seconds;
    double ca = c_v / next_volumn;
    return ca;
}

double Reservoir::predict_nitrate(const double &seconds, const double &load_in,
                                  const double &load_out, const double &ra,
                                  const double &rn, const double &next_volumn) {
    double c_v = status.c_nn * volumn + (load_in - load_out) * seconds * 1e-8 -
                 rn * volumn * seconds + ra * volumn * seconds;
    double cn = c_v / next_volumn;
    return cn;
}

void Reservoir::predict(const double &dt, const double &flow_in,
                        ...
                        const double &load_nit_out, const double &ro,
                        const double &ra, const double &rn) {
    double seconds     = dt * 86400;
    double next_volumn = predict_volumn(seconds, flow_in, flow_out);
    double next_wl = volumn_to_wl(next_volumn);
    double next_co =
        predict_organic(seconds, load_org_in, load_org_out, ro, next_volumn);
    double next_ca = predict_ammonia(seconds, load_amm_in, load_amm_out, ro, ra,
                                     next_volumn);
